		...
		float *best_d)
{
	...
	float dist_new = compute_distance(a,b,rows,cols,channels,patch_size,x,y,bx_new,by_new);
		...
		int search_radius)
{
	...
	nnd[row*cols + col] = compute_distance(a,b,rows,cols,channels,patch_size,col,row,init_x,init_y) ; 
