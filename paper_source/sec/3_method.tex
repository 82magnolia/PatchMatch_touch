...
obtain a homography that gives an initial transfer of the reference tactile video
...
keypoints and therefore a far more stable estimate of everything but the translation --- rotation,
scale, shear and the projective terms; but that estimate is expressed in wide-field pixels, and
rescaling it down to the narrow field of view that the tactile video actually covers amplifies any
error in its translation component. We therefore first solve Eq.~\ref{eq:homography} at the
matching scale, rescale the result to video resolution, and strip its translation so that the
remaining transform $\mathbf{H}_0$ fixes the image centre. We then warp the reference normal map
by $\mathbf{H}_0$ and re-match against the query at video scale; since everything but the
translation is already accounted for, the residual displacement is a pure shift, and we take its
component-wise median over the matches,
\begin{equation}
