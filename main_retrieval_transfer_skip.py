...
def main():
    ...
    parser.add_argument("--use_video_concate", default=None, type=int,
                             help="Concatenate video to NNF multiple frame transfer simultaneously (All frame : -1)")
    parser.add_argument("--downsample", default=1, type=int,
                             help="Downsampling stride when concatenating the entire video")
    ...
    for entry in tqdm(retrieval_results, desc="Transferring"):
        ...
        if args.use_keyframe:
            ...
            # If -1, group the entire video. If positive, split into chunks of that size
            chunk_size = len(ref_frames) if args.use_video_concate == -1 else args.use_video_concate
            
            # Pre-allocate arrays to store all rendered frames in chronological order
            transferred = [None] * len(ref_frames)
            valid_q_frames = [None] * len(ref_frames)
            ref_contact_masks = [None] * len(ref_frames)
            ...
            for start_idx in tqdm(range(0, len(ref_frames), chunk_size), desc="Transferring chunks", leave=False):
                ...
                # All frame indices included in the current chunk
                chunk_all_indices = list(range(start_idx, end_idx))
                
                # Apply downsampling: Extract search frames at args.downsample intervals within the chunk
                chunk_search_indices = chunk_all_indices[::args.downsample]
                
                search_frames = [ref_frames[idx] for idx in chunk_search_indices]
                all_frames = [ref_frames[idx] for idx in chunk_all_indices]
                
                # =================================================================
                # 1. Search Phase: Find optimal NNF for the current chunk using downsampled frames
                # =================================================================
                concat_search_ref = np.concatenate(search_frames, axis=-1)
                
                # Use the last result of the previous chunk (or initial background) as init for a smooth transition
                init_frame = ref_frames[0] if start_idx == 0 else transferred[start_idx - 1]
                concat_search_init = np.concatenate([init_frame] * len(search_frames), axis=-1)
                
                search_contact_mask = None
                if args.use_ref_contact_mask:
                    masks = [compute_contact_mask(f, base_frame, args.ref_contact_threshold, 
                                                  args.ref_contact_blur_sigma, args.ref_contact_morph_radius) 
                             for f in search_frames]
                    search_contact_mask = np.max(masks, axis=0) # Combine all shadow/contact areas (Union)
                ...
                if args.em:
                    # Feed the thick search matrix to generate a temporally consistent backbone
                    _, pm, _ = em_transfer_frame(
                        query_static, ref_static, concat_search_ref, concat_search_init,
                        current_em_iters, args.patch_size, args.iters,
                        ref_contact_mask=search_contact_mask,
                        ref_static_mask=ref_static_mask,
                        init_nnf=pass_nnf,
                        use_accel=args.use_accel)
                else:
                    max_radius = int(max(query_static.shape[:2]))
                    pm = PatchMatchSingle(query_static, ref_static, patch_size=args.patch_size, init_nnf=pass_nnf)
                    pm.propagate(iters=args.iters, rand_search_radius=max_radius)
                ...
                # =================================================================
                # 2. Reconstruct Phase: Apply the found NNF to "ALL" frames in the current chunk
                # =================================================================
                concat_all_ref = np.concatenate(all_frames, axis=-1)
                
                all_valids = []
                for idx in chunk_all_indices:
                    frame = ref_frames[idx]
                    valid = np.ones((frame.shape[0], frame.shape[1], 1), dtype=np.float32)
                    if args.use_ref_contact_mask:
                        c_mask = compute_contact_mask(frame, base_frame, args.ref_contact_threshold,
                                                      args.ref_contact_blur_sigma, args.ref_contact_morph_radius)
                ...
                concat_all_valid = np.concatenate(all_valids, axis=-1)
                
                # Ultra-fast parallel rendering
                concat_all_out = pm.reconstruct_avg(concat_all_ref, patch_size=1)
                concat_all_valid_q = pm.reconstruct_avg(concat_all_valid, patch_size=1)
                
                # =================================================================
                # 3. Split & Blend: Split the concatenated output and place frames in their correct positions
                # =================================================================
                out_frames = np.split(concat_all_out, len(all_frames), axis=-1)
                valid_q_frames_split = np.split(concat_all_valid_q, len(all_frames), axis=-1)
                
                for j, (out_f, valid_q_f) in enumerate(zip(out_frames, valid_q_frames_split)):
                    global_i = chunk_all_indices[j]
