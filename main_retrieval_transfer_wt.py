...
def em_transfer_frame(query_static, ref_static, ref_frame, init_estimate,
    ...
    if current_nnf is None and downsample > 1:
        ...
        print("\nLow res nnf: ", f"{(time.time() - start):.4f}")
    ...
    for em_step in range(em_iters):
        ...
        print(f"EM {em_step} step nnf: ", f"{(time.time() - nnf_start):.4f}")
        ...
        print(f"EM {em_step} step Recon.: ", f"{(time.time() - recon_start):.4f}")
    ...
    print("EM iter: ", f"{(time.time() - start):.4f}")
    print("Patchmatch Recon. avg: ", f"{(time.time() - start_time):.4f}")
...
def em_transfer_frame_downsample(query_static, ref_static, ref_frame, init_estimate,
    ...
    if downsample > 1:
        ...
        print("\nLow res EM nnf: ", f"{(time.time() - start):.4f}")
        ...
        print("Recon. avg: ", f"{(time.time() - start):.4f}")
        print("Patchmatch total Recon.: ", f"{(time.time() - start_time):.4f}")
        ...
        for em_step in range(em_iters):
            ...
            valid_q = (np.atleast_3d(pm.reconstruct_avg(valid, patch_size=1))[..., :1] > 0.5).astype(np.float32)
            estimate = valid_q * estimate + (1.0 - valid_q) * init_orig
        print("Patchmatch total Recon.: ", f"{(time.time() - start):.4f}")
        return estimate, pm, valid_q, pm
...
def main():
    ...
    parser.add_argument("--use_wt", default=None, choices=["LLL", "guided", "all"],
                        help="Apply Wavelet Transform before PatchMatch. 'LLL' for low-freq only, 'all' for concat all bands.")
    parser.add_argument("--wt_level", default=1, type=int, help="Number of Wavelet Transform levels")
    ...
    for entry in tqdm(retrieval_results, desc="Transferring"):
        ...
        if args.use_wt is not None:
            from wavelet_transform import forward_2d_wt
            query_static = forward_2d_wt(query_static, method=args.use_wt, level=args.wt_level)
            ref_static = forward_2d_wt(ref_static, method=args.use_wt, level=args.wt_level)
            print(f"  [WT Applied] Static images transformed. New shape: {query_static.shape}")
        ...
        # Video Wavelet Transform (3D)
        if args.use_wt is not None:
            from wavelet_transform import forward_3d_wt
            base_frames_arr = np.array([base_frame] * (2 ** args.wt_level), dtype=np.float32)
            base_frame = forward_3d_wt(base_frames_arr, method=args.use_wt, level=args.wt_level)[0]

            if args.use_wt == 'guided':
                base_frame = base_frame[0]
                ref_video_arr = np.array(ref_frames, dtype=np.float32)
                ref_video_wt, v_hf = forward_3d_wt(ref_video_arr, method='guided')
                transferred_hf = {k: [] for k in v_hf.keys()}
            else:
                ref_video_arr = np.array(ref_frames, dtype=np.float32)
                ref_video_wt = forward_3d_wt(ref_video_arr, method=args.use_wt, level=args.wt_level)
            
            ref_frames = [ref_video_wt[i] for i in range(ref_video_wt.shape[0])]
            fps = fps / (2.0 ** args.wt_level) 
            print(f"  [WT Applied] Video transformed. New frames: {len(ref_frames)}, Shape: {ref_frames[0].shape}")
            ref_frames[0] = base_frame
        ...
        if args.use_mask:
            ...
            if not osp.exists(mask_path):
                ...
                if args.use_wt is not None:
                    mask_arr = np.array(mask_frames, dtype=np.float32)
                    
                    from wavelet_transform import forward_3d_wt
                    mask_wt = forward_3d_wt(mask_arr, method='LLL', level=args.wt_level)
                    
                    # When converting to 3D Haar, the value doubles for each axis, increasing by a total of 8 times.
                    # Normalize the mask values ​​back to between 0.0 and 1.0 for synthesis (Alpha Blending)
                    scale_factor = np.sqrt(8) ** args.wt_level
                    mask_wt = np.clip(mask_wt / scale_factor, 0.0, 1.0)
                    
                    mask_frames = [mask_wt[i] for i in range(mask_wt.shape[0])]
                    print(f"  [WT Mask] Mask temporally and spatially transformed. New length: {len(mask_frames)}, Shape: {mask_frames[0].shape}")
        ...
        if False:
            ...
            for start_idx in tqdm(range(0, len(ref_frames), chunk_size), desc="Transferring chunks", leave=False):
                ...
                if args.use_keyframe:
                    ref_contact_masks = [None] * len(ref_frames)
                    deformation_scores = []
                    for i, ref_frame in enumerate(chunk_frames):
                        if args.use_ref_contact_mask:
                            mask = compute_contact_mask(
                                ref_frame, base_frame, args.ref_contact_threshold,
                                args.ref_contact_blur_sigma, args.ref_contact_morph_radius)
                    ...
                    anchor_idx = int(np.argmax(deformation_scores))
                    print(f"  [Info] Anchor frame selected: {anchor_idx}/{len(ref_frames)-1} (Max deformation)")
                    current_ref_frame = chunk_frames[anchor_idx]
                    init = ref_frames[anchor_idx]
                ...
                if args.em:
                    ...
                    if args.only_normal:
                        ...
                        _, pm_low, _, pm = em_transfer_frame_downsample(
                            query_static, ref_static, current_ref_frame, init,
                            current_em_iters, args.patch_size, args.iters,
                            ref_contact_mask=ref_contact_mask,
                            ref_static_mask=ref_static_mask,
                ...
                if args.use_wt == 'guided':
                    for k in v_hf.keys():
                        chunk_hf = np.concatenate(v_hf[k][start_idx:end_idx], axis=-1)
                        recon_hf_concat = pm.reconstruct_avg(chunk_hf, patch_size=1)
                        
                        recon_hf_frames = np.split(recon_hf_concat, len(chunk_frames), axis=-1)
                        transferred_hf[k].extend(recon_hf_frames)
                ...
                for j, (out_f, valid_q_f) in enumerate(zip(out_frames, valid_q_frames_chunk)):
                    ...
                    if mask_frames is not None:
                        ...
                        if mask.shape[:2] != final_out.shape[:2]:
                            mask = cv2.resize(mask, (final_out.shape[1], final_out.shape[0]), interpolation=cv2.INTER_NEAREST)
            ...
            if args.em:
                ...
                for i, ref_frame in enumerate(tqdm(ref_frames, desc="Transferring frames", leave=False)):
                    ...
                    if args.use_accel is False:
                        prev_nnf = None
                    start = time.time()
                    output, last_pm, valid_q, _ = em_transfer_frame_downsample(
                    ...
                    if args.use_wt == 'guided':
                        for k in v_hf.keys():
                            hf_frame = v_hf[k][i]
                            recon_hf = last_pm.reconstruct_avg(hf_frame, patch_size=1)
                            transferred_hf[k].append(recon_hf)
        ...
        if args.use_wt is not None:
            from wavelet_transform import inverse_3d_wt
            print("  [WT Inverse] Reconstructing video from WT bands...")            
            trans_arr = np.array(transferred, dtype=np.float32)
            
            # Perform inverse transform -> Restores original resolution and time (T)
            if args.use_wt == 'guided':
                trans_hf_dict = {k: np.array(transferred_hf[k], dtype=np.float32) for k in transferred_hf.keys()}
                
                recon_arr = inverse_3d_wt((trans_arr, trans_hf_dict), method='guided')

                ref_vid_arr = np.array(ref_frames, dtype=np.float32)
                recon_ref = inverse_3d_wt((ref_vid_arr, v_hf), method='guided')
            else:
                recon_arr = inverse_3d_wt(trans_arr, method=args.use_wt, level=args.wt_level)

                ref_vid_arr = np.array(ref_frames, dtype=np.float32)
                recon_ref = inverse_3d_wt(ref_vid_arr, method=args.use_wt)
            
            # Clip values as inverse transform math might slightly exceed the [0, 1] range
            recon_arr = np.clip(recon_arr, 0.0, 1.0)
            
            # Revert back to list of frames and restore FPS
            transferred = [recon_arr[i] for i in range(recon_arr.shape[0])]
            fps = fps * (2.0 ** args.wt_level)
            
            # Reconstruct ref_frames back to original for comparison video saving
            recon_ref = np.clip(recon_ref, 0.0, 1.0)
            ref_frames = [recon_ref[i] for i in range(recon_ref.shape[0])]
        ...
        if args.eval:
            ...
            print(f"{metrics_accel['MSE']:.5f}\t{metrics_accel['PSNR']:.2f}\t{metrics_accel['SSIM']:.4f}\t{metrics_accel['LPIPS']:.4f}\t{runtime:.3f}\n")
