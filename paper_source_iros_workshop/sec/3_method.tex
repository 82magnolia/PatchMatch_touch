...
The initial transfer is geometrically plausible but may be physically inaccurate.
Specifically, the initial transfer carries the gel dynamics the \emph{reference press} happened to exhibit,
inherits any residual misalignment left by Eq.~\ref{eq:homography}, and 
knows nothing about the surface geometry at the query pose beyond what survived the homography-based alignment.
Our final step corrects these artifacts using a neural network. Since we target
...
ConvNeXt-style~\cite{convnext} spatio-temporal branch and a per-frame token branch, and merges them in a shared
...
Following the standard diffusion-model recipe~\cite{ldm}, a fixed sinusoidal embedding of $\tau_t$
is passed through a small multi-layer perceptron and mapped to a per-stage scale and shift, applied
as feature-wise linear modulation (FiLM)~\cite{film}:
\begin{equation}
