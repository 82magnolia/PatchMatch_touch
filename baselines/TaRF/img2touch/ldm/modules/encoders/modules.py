...
class ResnetEncoder(pl.LightningModule):
    def __init__(self,
        ...
        if model_name == 'resnet18':
            # The TaRF diffusion checkpoint contains these weights. Avoid an
            # implicit network download during deterministic batch inference.
...
class ResnetRGBDEncoder(pl.LightningModule):
    def __init__(self,
        ...
        super().__init__()
        if model_name == 'resnet18':
            # These parameters are restored from the TaRF diffusion
            # checkpoint, so inference must not download ImageNet weights.
            self.resnet = models.resnet18(weights=None)
            # rgbdb
