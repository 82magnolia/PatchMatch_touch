...
def synthesis(args):
    try:
        ...
        new_h, new_w = int(args.scale), int(args.scale)
        print(img.shape)
        #new_img = img
        new_img = textureSynthesis.Construct(img, [args.block_size, args.block_size], args.overlap, int(new_h/4), int(new_w/4), args.tolerance)
        print(new_img.shape)
        new_img = textureSynthesis.Construct(new_img, [args.block_size*4, args.block_size*4], args.overlap, new_h, new_w, args.tolerance)
        print(new_img.shape)
        ...
        img_to_save.save("outputs/" + "normal_quilt_img.png")
