...
parser.add_argument('--TDF_num', type=str, default=None, help='Taxim simulate using height npy file')
...
def TouchNet_train(args):
    ...
    data_folder = os.environ.get("RQT_CALIB_DIR", osp.join("calibs"))
...
def TouchNet_eval(args):
    ...
    if TDF is not None:
        vertex_coordinates_raw = np.asarray([np.load(os.path.join("../outputs", "viewspace_point.npy"))])
    N = vertex_coordinates_raw.shape[0]
    ...
    sim = tac_sim(data_folder, args.obj_path, obj, args.obj_scale_factor)
    if TDF is not None:
        GT_scale = args.obj_scale_factor
    else:
        GT_scale = args.obj_scale_factor / 2.5
    GT_sim = tac_sim(data_folder, args.obj_path, obj, GT_scale)
    press_depth = args.depth
    dx = 0
    dy = 0
    taxim = TaximRender(data_folder)
    ...
    for i in pbar:         
        ...
        # Ground Truth (GT) (Simulator)
        raw_vertices = vertex_coordinates_raw
        height_map, gel_map, contact_mask, raw_color_map, _, vis_raw_normal_map, raw_height_map = sim.generateHeightMap(gelpad_model_path, press_depth * 20, dx, dy, contact_point=raw_vertices[i], contact_theta=np.pi)
        
        GT_height_map,GT_gelmap,GT_contact_mask,_,_,GT_normal_map,_ = GT_sim.generateHeightMap(gelpad_model_path, press_depth * 20, dx, dy, contact_point=raw_vertices[i], contact_theta=np.pi)
        GT_contact_mask[:,:] = 1
        GT_heightMap, GT_contact_mask, GT_contact_height = GT_sim.deformApprox(press_depth * 20, GT_height_map, GT_gelmap, GT_contact_mask)
        GT_sim_img, _ = GT_sim.simulating(GT_heightMap, GT_contact_mask, GT_contact_height, shadow=True)
        
        
        if TDF is not None:
            height_map = np.load(os.path.join('..', 'outputs', f'{TDF}_height_map_resize.npy'))
            height_map += 200
            #print(f"Min: {height_map.min()}, Max: {height_map.max()}")
            contact_mask[:,:] = 1

        heightMap, contact_mask, contact_height = sim.deformApprox(press_depth * 20, height_map, gel_map, contact_mask)
        sim_img, shadow_sim_img = sim.simulating(heightMap, contact_mask, contact_height, shadow=True)

        if TDF is not None:
            img_savePath = os.path.join('../outputs', 'sim_img.png')
            cv2.imwrite(img_savePath, sim_img)

            GT_viewspace_normal = (GT_normal_map * 255).astype(np.uint8)
            GT_viewspace_savePath = os.path.join('../outputs', 'GT_viewspace_normal.png')
            cv2.imwrite(GT_viewspace_savePath, cv2.cvtColor(cv2.flip(GT_viewspace_normal, 1), cv2.COLOR_RGB2BGR))

            GT_sim_savePath = os.path.join('../outputs', 'GT_sim_img.png')
            cv2.imwrite(GT_sim_savePath, cv2.flip(GT_sim_img, 1))
        else:
            raw_normal_img = (vis_raw_normal_map * 255).astype(np.uint8)
            TDF_normal_savePath = os.path.join('../outputs', 'tactile_normal_img.png')
            cv2.imwrite(TDF_normal_savePath, cv2.cvtColor(raw_normal_img, cv2.COLOR_RGB2BGR))

            #GT - tactile normal image
            GT_normal_map = (GT_normal_map * 255).astype(np.uint8)
            GT_normal_savePath = os.path.join('../outputs', 'GT_tactile_normal_img.png')
            cv2.imwrite(GT_normal_savePath, cv2.cvtColor(GT_normal_map, cv2.COLOR_RGB2BGR))
