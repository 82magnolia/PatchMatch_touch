...
class OrbitCamera:
    ...
    def from_angle(self, target, normal, is_degree=True):
        ...
        '''
        campos = target + normal      #contact normal

        #rot_mat = look_at(campos, np.zeros([3], dtype=np.float32))
        rot_mat = look_at(campos, target)
        self.rot = Rotation.from_matrix(rot_mat)

        #self.radius = np.linalg.norm(campos - target)
        self.center = -target
