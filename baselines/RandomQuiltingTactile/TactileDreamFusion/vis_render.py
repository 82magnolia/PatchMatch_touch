...
class GUI:
    def __init__(self, opt):
        ...
        self.W = opt.W
        self.H = opt.H
        self.cam = OrbitCamera(opt.W, opt.H, r=opt.radius, fovy=opt.fovy, proj_mode='orthographic', view_volume_size=opt.scale)
...
def main():
    ...
    parser.add_argument('--W', type=int, default=640, help="GUI width")
    parser.add_argument('--H', type=int, default=480, help="GUI height")
    ...
    parser.add_argument('--scale', type=float, default=1.5, help="distance between object from camera(render scale)")
    parser.add_argument('--view_path', type=str, default=None, help="visual rendering from random point")
    ...
    pcd = o3d.io.read_point_cloud(opt.view_path)
    pcd_vertices = np.asarray(pcd.points)
    random_index = np.random.choice(pcd_vertices.shape[0], size=1, replace=False)
    random_point = pcd_vertices[random_index][0]
    np.save(os.path.join("../outputs", "viewspace_point.npy"), random_point)
    ...
    if opt.save is not None:
        ...
        for ele in tqdm.tqdm(elevation):
            for azi in tqdm.tqdm(azimuth):
                point_normal = contact_point_normal(opt.mesh, random_point)
                gui.cam.from_angle(random_point, point_normal)
                ...
                cv2.imwrite(os.path.join(opt.save, f'{opt.mode}.png'), image)
