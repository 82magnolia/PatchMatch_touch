#!/usr/bin/env bash
# Full-pipeline evaluation (retrieve -> feature-match transfer -> ReBotNet XS
# residual-mode refinement) over every session in log/real_data_gt_retrieval, via
# transfer_pipeline.py end to end. Uses the pre-trained checkpoint produced by
# train_refine_scripts/train_rebot_pseudo_mini_residual/train_rebot_XS.sh
...
#
# Usage: bash pipeline_scrips/eval_pipeline_real_data_gt_retrieval_residual/eval_pipeline_XS.sh <gpu_id> [matcher] [masked]
...
# override e.g. `NUM_THREADS=7 bash eval_pipeline_XS.sh <gpu_id>` when running
...
GPU=${1:?Usage: bash eval_pipeline_XS.sh <gpu_id> [matcher] [masked]}
MATCHER="${2:-loftr}"
USE_MASK="${3:-}"
MODEL_SIZE=rebot_XS
...
# train_rebot_pseudo_mini_residual/train_rebot_XS.sh's convention.
# Checkpoint dir always keeps _residual, with the matcher suffix appended after it.
...
CHECKPOINT="$PROJECT_ROOT/log/rebot_checkpoints_XS_pseudo_mini${NAME_SUFFIX}/best.pth"
SESSIONS_BASE="$PROJECT_ROOT/log/real_data_gt_retrieval"
OUT_BASE="$PROJECT_ROOT/log/pipeline_eval_XS_real_data_gt_retrieval${NAME_SUFFIX}"

if [ ! -f "$CHECKPOINT" ]; then
    echo "Checkpoint not found: $CHECKPOINT" >&2
    echo "Train it first with train_refine_scripts/train_rebot_pseudo_mini_residual/train_rebot_XS.sh $MATCHER $USE_MASK" >&2
    exit 1
fi
