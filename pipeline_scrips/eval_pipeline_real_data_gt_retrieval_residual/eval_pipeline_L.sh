#!/usr/bin/env bash
# Full-pipeline evaluation (retrieve -> feature-match transfer -> ReBotNet L
...
# train_refine_scripts/train_rebot_pseudo_mini_residual/train_rebot_L.sh
...
#
# Usage: bash pipeline_scrips/eval_pipeline_real_data_gt_retrieval_residual/eval_pipeline_L.sh <gpu_id> [matcher] [masked]
...
# override e.g. `NUM_THREADS=7 bash eval_pipeline_L.sh <gpu_id>` when running
...
GPU=${1:?Usage: bash eval_pipeline_L.sh <gpu_id> [matcher] [masked]}
MATCHER="${2:-loftr}"
USE_MASK="${3:-}"
MODEL_SIZE=rebot_L
...
# train_rebot_pseudo_mini_residual/train_rebot_L.sh's convention.
...
CHECKPOINT="$PROJECT_ROOT/log/rebot_checkpoints_L_pseudo_mini${NAME_SUFFIX}/best.pth"
SESSIONS_BASE="$PROJECT_ROOT/log/real_data_gt_retrieval"
OUT_BASE="$PROJECT_ROOT/log/pipeline_eval_L_real_data_gt_retrieval${NAME_SUFFIX}"

if [ ! -f "$CHECKPOINT" ]; then
    echo "Checkpoint not found: $CHECKPOINT" >&2
    echo "Train it first with train_refine_scripts/train_rebot_pseudo_mini_residual/train_rebot_L.sh $MATCHER $USE_MASK" >&2
    exit 1
fi
