...
# train_refine_scripts/train_rebot_pseudo_mini/train_rebot_XS.sh
...
#
# Usage: bash pipeline_scrips/eval_pipeline_real_data_gt_retrieval/eval_pipeline_XS.sh <gpu_id> [matcher] [masked]
...
# train_rebot_pseudo_mini/train_rebot_XS.sh's convention.
...
CHECKPOINT="$PROJECT_ROOT/log/rebot_checkpoints_XS_pseudo_mini${SUFFIX}/best.pth"
SESSIONS_BASE="$PROJECT_ROOT/log/real_data_gt_retrieval"
OUT_BASE="$PROJECT_ROOT/log/pipeline_eval_XS_real_data_gt_retrieval${SUFFIX}"

if [ ! -f "$CHECKPOINT" ]; then
    echo "Checkpoint not found: $CHECKPOINT" >&2
    echo "Train it first with train_refine_scripts/train_rebot_pseudo_mini/train_rebot_XS.sh $MATCHER $USE_MASK" >&2
    exit 1
fi
