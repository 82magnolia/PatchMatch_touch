...
# train_refine_scripts/train_rebot_pseudo_mini/train_rebot_L.sh
...
#
# Usage: bash pipeline_scrips/eval_pipeline_real_data_gt_retrieval/eval_pipeline_L.sh <gpu_id> [matcher] [masked]
...
# train_rebot_pseudo_mini/train_rebot_L.sh's convention.
...
OUT_BASE="$PROJECT_ROOT/log/pipeline_eval_L_real_data_gt_retrieval${SUFFIX}"

if [ ! -f "$CHECKPOINT" ]; then
    echo "Checkpoint not found: $CHECKPOINT" >&2
    echo "Train it first with train_refine_scripts/train_rebot_pseudo_mini/train_rebot_L.sh $MATCHER $USE_MASK" >&2
    exit 1
fi
