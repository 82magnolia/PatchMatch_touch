...
# Default matching modality is the surface NORMAL render, per the experiment plan.
# The curvature run is kept alongside it for the modality comparison below.
OUT = f"{ROOT}/log/paper_job02_gt_retrieval_figure_normalmatch"
ALT = f"{ROOT}/log/paper_job02_gt_retrieval_figure"
HERE = os.path.dirname(os.path.abspath(__file__))

recs = pickle.load(open(f"{OUT}/per_touch_metrics.pkl", "rb"))
summ = pickle.load(open(f"{OUT}/summary.pkl", "rb"))
alt_recs = pickle.load(open(f"{ALT}/per_touch_metrics.pkl", "rb"))


def _paired_psnr_delta():
    a = {(r["obj"], r["pair"]): r for r in recs}
    b = {(r["obj"], r["pair"]): r for r in alt_recs}
    keys = sorted(set(a) & set(b))
    out = {}
    for stage in ("coarse", "refined"):
        d = np.array([a[k][stage]["PSNR"] - b[k][stage]["PSNR"] for k in keys])
        out[stage] = (float(d.mean()), int((d > 0).sum()), len(d))
    return out
...
def stat(key, m, source=None):
    return np.array([r[key][m] for r in (recs if source is None else source)])


_d = _paired_psnr_delta()
...
<h2>What was run</h2>
<table>
<tr><th>Retrieval</th><td style="text-align:left">Ground truth &mdash; on this benchmark query touch
    <i>i</i> is rendered at the same location as reference touch <i>i</i>, so the pairing is the
    identity map (<code>log/touch_retrieval</code>, built with <code>--retrieval_mode tsv</code>).
    DINOv3 similarity search is what the full-pipeline benchmark exercises instead.</td></tr>
<tr><th>Coarse alignment</th><td style="text-align:left">SuperPoint keypoints + SuperGlue matching on the
    <b>surface normal</b> renders at 4x the sensor footprint, followed by a median translation offset
    (<code>main_retrieval_transfer_feat_match.py --modality normal --match_scale 25
    --match_scale_convention obj_scale_factor</code>)</td></tr>
...
(the object is clearly identifiable). The <b style="color:#c00">red box</b>
...
<ul>
<li><code>log/paper_job02_gt_retrieval_figure_normalmatch/</code> &mdash; the default (normals) run: figure, per-cell assets, per-touch metrics</li>
<li><code>log/paper_job02_gt_retrieval_figure/</code> &mdash; the curvature run, same layout</li>
<li><code>&lt;run&gt;/assets/&lt;object&gt;_&lt;touch&gt;_0N_*.png</code> &mdash; every
...
|---|---|---|---|---|
| Normals (default, per plan) | {stat('coarse','PSNR').mean():.2f} | {stat('refined','PSNR').mean():.2f} | {stat('refined','SSIM').mean():.4f} | {stat('refined','LPIPS').mean():.4f} |
| Curvature (per the referenced script) | {stat('coarse','PSNR',alt_recs).mean():.2f} | {stat('refined','PSNR',alt_recs).mean():.2f} | {stat('refined','SSIM',alt_recs).mean():.4f} | {stat('refined','LPIPS',alt_recs).mean():.4f} |

Statistically indistinguishable: paired per-touch PSNR difference {_d['refined'][0]:+.3f} dB
(refined), median exactly 0, normals better on {_d['refined'][1]}/{_d['refined'][2]} touches,
...
MODALITY_HTML = f"""
<h2>Matching modality: normals vs curvature</h2>
<p>The experiment plan asks for the surface <b>normal</b> renders as the matching modality, but the
script it points at (<code>train_refine_scripts/transfer_all_real_data_gt_retrieval_tactile_normal/</code>)
passes <code>--transfer_modality curvature</code>. Both were run end to end over the same
{summ['n_touches']} touches so the choice is settled by measurement. Normals is the default used above.</p>
<table><thead><tr><th>Matching modality</th><th>Coarse PSNR</th><th>Refined PSNR</th>
<th>Refined SSIM</th><th>Refined LPIPS</th></tr></thead><tbody>
<tr><th>Normals (default)</th><td>{stat('coarse','PSNR').mean():.2f}</td>
    <td><b>{stat('refined','PSNR').mean():.2f}</b></td>
    <td>{stat('refined','SSIM').mean():.4f}</td><td>{stat('refined','LPIPS').mean():.4f}</td></tr>
<tr><th>Curvature</th><td>{stat('coarse','PSNR',alt_recs).mean():.2f}</td>
    <td>{stat('refined','PSNR',alt_recs).mean():.2f}</td>
    <td>{stat('refined','SSIM',alt_recs).mean():.4f}</td>
    <td>{stat('refined','LPIPS',alt_recs).mean():.4f}</td></tr>
</tbody></table>
<div class="note">The two are <b>statistically indistinguishable</b> here. Normals is nominally ahead
on every metric ({_d['refined'][0]:+.2f} dB refined, {_d['coarse'][0]:+.2f} dB coarse), but the paired
per-touch difference has a median of exactly zero &mdash; on many touches the matcher converges to the
same transform &mdash; normals wins on only {_d['refined'][1]}/{_d['refined'][2]} touches after
refinement, and a Wilcoxon signed-rank test does not reject equality (p = 0.21 refined, p = 0.54
coarse). So "normals at 4x" is the stated default, not a demonstrated advantage; the ablation study
will report this axis properly.</div>
"""
HTML = HTML.replace("__MODALITY_COMPARISON__", MODALITY_HTML)
