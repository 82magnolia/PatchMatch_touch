...
#
# Usage: bash train_refine_scripts/finetune_rebot_real_data_gt_retrieval_residual_cond/finetune_rebot_S.sh <gpu_id> [matcher] [finetune_mode] [cond_mode]
...
GPU=${1:?Usage: bash finetune_rebot_S.sh <gpu_id> [matcher] [finetune_mode] [cond_mode]}
MATCHER="${2:-loftr}"
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/finetune.py" \
    --pretrained    "$PROJECT_ROOT/log/rebot_checkpoints_S_pseudo_mini${CKPT_NAME}_cond-${COND_MODE}" \
    --transfer_dir  "$PROJECT_ROOT/log/transfer_pipeline_real_data_gt_retrieval${TRANSFER_SUFFIX}" \
    --save_dir      "$PROJECT_ROOT/log/rebot_finetune_S_real_data_gt_retrieval_residual${TRANSFER_SUFFIX}_${FINETUNE_MODE}_cond-${COND_MODE}" \
    ...
    --wandb_run_name "finetune_S_real_residual${TRANSFER_SUFFIX}_${FINETUNE_MODE}_cond-${COND_MODE}_bs8_lr5e-5"
