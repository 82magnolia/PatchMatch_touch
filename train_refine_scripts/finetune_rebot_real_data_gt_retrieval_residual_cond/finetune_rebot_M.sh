...
# train_refine_scripts/train_rebot_pseudo_mini_residual_cond/train_rebot_M.sh (same cond_mode ->
...
#
# Usage: bash train_refine_scripts/finetune_rebot_real_data_gt_retrieval_residual_cond/finetune_rebot_M.sh <gpu_id> [matcher] [finetune_mode] [cond_mode]
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/finetune.py" \
    ...
    --save_dir      "$PROJECT_ROOT/log/rebot_finetune_M_real_data_gt_retrieval_residual${TRANSFER_SUFFIX}_${FINETUNE_MODE}_cond-${COND_MODE}" \
    ...
    --wandb_run_name "finetune_M_real_residual${TRANSFER_SUFFIX}_${FINETUNE_MODE}_cond-${COND_MODE}_bs8_lr5e-5"
