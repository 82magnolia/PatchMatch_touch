...
# train_refine_scripts/train_rebot_pseudo_mini_residual/train_rebot_S.sh and adapts it to the
...
#
# Usage: bash train_refine_scripts/finetune_rebot_real_data_gt_retrieval_residual/finetune_rebot_S.sh <gpu_id> [matcher] [refined] [finetune_mode]
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/finetune.py" \
    --pretrained    "$PROJECT_ROOT/log/rebot_checkpoints_S_pseudo_mini_residual${CKPT_SUFFIX}" \
    --transfer_dir  "$PROJECT_ROOT/log/transfer_pipeline_real_data_gt_retrieval${TRANSFER_SUFFIX}" \
    --save_dir      "$PROJECT_ROOT/log/rebot_finetune_S_real_data_gt_retrieval_residual${TRANSFER_SUFFIX}_${FINETUNE_MODE}" \
    ...
    --wandb_run_name "finetune_S_real_residual${TRANSFER_SUFFIX}_${FINETUNE_MODE}_bs8_lr5e-5"
