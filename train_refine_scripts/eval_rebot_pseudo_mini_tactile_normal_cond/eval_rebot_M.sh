...
#
# Usage: bash train_refine_scripts/eval_rebot_pseudo_mini_tactile_normal_cond/eval_rebot_M.sh <gpu_id> [cond_mode]
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/eval.py" \
    ...
    --checkpoint   "$PROJECT_ROOT/log/rebot_checkpoints_M_pseudo_mini_tactile_normal_cond-${COND_MODE}/best.pth" \
    --model_size   $MODEL_SIZE \
    --video_type   tactile_normal \
    --save_dir     "$PROJECT_ROOT/log/rebot_eval_M_pseudo_mini_tactile_normal_cond-${COND_MODE}" \
