...
#
# Usage: bash train_refine_scripts/eval_rebot_pseudo_mini_tactile_normal_residual_cond/eval_rebot_S.sh <gpu_id> [matcher] [cond_mode]
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/eval.py" \
    ...
    --checkpoint   "$PROJECT_ROOT/log/rebot_checkpoints_S_pseudo_mini_tactile_normal_residual${SUFFIX}_cond-${COND_MODE}/best.pth" \
    --model_size   $MODEL_SIZE \
    --video_type   tactile_normal \
    --save_dir     "$PROJECT_ROOT/log/rebot_eval_S_pseudo_mini_tactile_normal_residual${SUFFIX}_cond-${COND_MODE}" \
