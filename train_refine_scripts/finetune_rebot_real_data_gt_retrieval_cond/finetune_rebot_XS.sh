...
# train_refine_scripts/train_rebot_pseudo_mini_cond/train_rebot_XS.sh (same cond_mode ->
...
#
# Usage: bash train_refine_scripts/finetune_rebot_real_data_gt_retrieval_cond/finetune_rebot_XS.sh <gpu_id> [matcher] [finetune_mode] [cond_mode]
...
GPU=${1:?Usage: bash finetune_rebot_XS.sh <gpu_id> [matcher] [finetune_mode] [cond_mode]}
MATCHER="${2:-loftr}"
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/finetune.py" \
    --pretrained    "$PROJECT_ROOT/log/rebot_checkpoints_XS_pseudo_mini${CKPT_NAME}_cond-${COND_MODE}" \
    --transfer_dir  "$PROJECT_ROOT/log/transfer_pipeline_real_data_gt_retrieval${TRANSFER_SUFFIX}" \
    --save_dir      "$PROJECT_ROOT/log/rebot_finetune_XS_real_data_gt_retrieval${TRANSFER_SUFFIX}_${FINETUNE_MODE}_cond-${COND_MODE}" \
    ...
    --wandb_run_name "finetune_XS_real${TRANSFER_SUFFIX}_${FINETUNE_MODE}_cond-${COND_MODE}_bs8_lr5e-5"
