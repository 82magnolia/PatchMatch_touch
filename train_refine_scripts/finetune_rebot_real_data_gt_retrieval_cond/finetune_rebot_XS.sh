...
#
# Usage: bash train_refine_scripts/finetune_rebot_real_data_gt_retrieval_cond/finetune_rebot_XS.sh <gpu_id> [matcher] [finetune_mode] [cond_mode] [data_mode]
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/finetune.py" \
    ...
    --save_dir      "$PROJECT_ROOT/log/rebot_finetune_XS_real_data_gt_retrieval${TRANSFER_SUFFIX}_${FINETUNE_MODE}_cond-${COND_MODE}${DATA_SUFFIX}" \
    ...
    --wandb_run_name "finetune_XS_real${TRANSFER_SUFFIX}_${FINETUNE_MODE}_cond-${COND_MODE}${DATA_SUFFIX}_bs8_lr5e-5"
