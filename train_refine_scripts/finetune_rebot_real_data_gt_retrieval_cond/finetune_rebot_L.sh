...
# train_refine_scripts/train_rebot_pseudo_mini_cond/train_rebot_L.sh (same cond_mode ->
...
#
# Usage: bash train_refine_scripts/finetune_rebot_real_data_gt_retrieval_cond/finetune_rebot_L.sh <gpu_id> [matcher] [finetune_mode] [cond_mode] [data_mode]
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/finetune.py" \
    ...
    --save_dir      "$PROJECT_ROOT/log/rebot_finetune_L_real_data_gt_retrieval${TRANSFER_SUFFIX}_${FINETUNE_MODE}_cond-${COND_MODE}${DATA_SUFFIX}" \
    ...
    --wandb_run_name "finetune_L_real${TRANSFER_SUFFIX}_${FINETUNE_MODE}_cond-${COND_MODE}${DATA_SUFFIX}_bs4_lr5e-5"
