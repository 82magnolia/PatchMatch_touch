#!/usr/bin/env bash
# Pretrain a CONDITIONED ReBotNet M (residual) on gelsight_pseudo_mini
...
#
# Usage: bash train_refine_scripts/train_rebot_pseudo_mini_tactile_normal_residual_cond/train_rebot_M.sh <gpu_id> [matcher] [cond_mode]
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/train.py" \
    ...
    --save_dir      "$PROJECT_ROOT/log/rebot_checkpoints_M_pseudo_mini_tactile_normal_residual${SUFFIX}_cond-${COND_MODE}" \
