#!/usr/bin/env bash
# Pretrain a CONDITIONED ReBotNet L on gelsight_pseudo_mini in sim.
...
#
# Usage: bash train_refine_scripts/train_rebot_pseudo_mini_cond/train_rebot_L.sh <gpu_id> [matcher] [cond_mode]
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/train.py" \
    ...
    --save_dir      "$PROJECT_ROOT/log/rebot_checkpoints_L_pseudo_mini${CKPT_NAME}_cond-${COND_MODE}" \
    ...
    --wandb_run_name "${MODEL_SIZE}_pseudo_mini${CKPT_NAME}_cond-${COND_MODE}_bs4_lr2e-4"
