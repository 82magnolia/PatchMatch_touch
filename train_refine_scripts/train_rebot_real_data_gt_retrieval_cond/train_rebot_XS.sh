#!/usr/bin/env bash
# Train a CONDITIONED ReBotNet XS FROM SCRATCH (random init) on real-data
# (transferred -> query) pairs. This is the from-scratch counterpart of
# train_refine_scripts/finetune_rebot_real_data_gt_retrieval_cond/finetune_rebot_XS.sh:
...
#
# Usage: bash train_refine_scripts/train_rebot_real_data_gt_retrieval_cond/train_rebot_XS.sh <gpu_id> [matcher] [cond_mode] [data_mode]
...
GPU=${1:?Usage: bash train_rebot_XS.sh <gpu_id> [matcher] [cond_mode] [data_mode]}
MATCHER="${2:-loftr}"
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/train.py" \
    ...
    --save_dir      "$PROJECT_ROOT/log/rebot_scratch_XS_real_data_gt_retrieval${TRANSFER_SUFFIX}_cond-${COND_MODE}${DATA_SUFFIX}" \
    ...
    --wandb_run_name "scratch_XS_real${TRANSFER_SUFFIX}_cond-${COND_MODE}${DATA_SUFFIX}_bs8_lr2e-4"
