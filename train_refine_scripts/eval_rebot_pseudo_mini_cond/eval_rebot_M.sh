#!/usr/bin/env bash
# Evaluate a CONDITIONED ReBotNet M trained on gelsight_pseudo_mini data
# (see train_refine_scripts/train_rebot_pseudo_mini_cond/train_rebot_M.sh).
#
# Usage: bash train_refine_scripts/eval_rebot_pseudo_mini_cond/eval_rebot_M.sh <gpu_id> [matcher] [cond_mode]
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/eval.py" \
    ...
    --checkpoint   "$PROJECT_ROOT/log/rebot_checkpoints_M_pseudo_mini${CKPT_NAME}_cond-${COND_MODE}/best.pth" \
    --model_size   $MODEL_SIZE \
    --save_dir     "$PROJECT_ROOT/log/rebot_eval_M_pseudo_mini${CKPT_NAME}_cond-${COND_MODE}" \
