#!/usr/bin/env bash
# Evaluate ReBotNet S variant trained on gelsight_pseudo_mini data.
# Usage: bash train_refine_scripts/eval_rebot_pseudo_mini/eval_rebot_S.sh <gpu_id> [matcher] [masked]
...
GPU=${1:?Usage: bash eval_rebot_S.sh <gpu_id> [matcher] [masked]}
MATCHER="${2:-loftr}"
USE_MASK="${3:-}"
MODEL_SIZE=rebot_S
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/eval.py" \
    ...
    --save_dir     "$PROJECT_ROOT/log/rebot_eval_S_pseudo_mini${SUFFIX}" \
    --video_save \
    --save_gt
