#!/usr/bin/env bash
# Evaluate ReBotNet M variant trained on gelsight_pseudo_mini data.
# Usage: bash train_refine_scripts/eval_rebot_pseudo_mini/eval_rebot_M.sh <gpu_id> [matcher] [masked]
...
GPU=${1:?Usage: bash eval_rebot_M.sh <gpu_id> [matcher] [masked]}
MATCHER="${2:-loftr}"
USE_MASK="${3:-}"
MODEL_SIZE=rebot_M
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/eval.py" \
    ...
    --save_dir     "$PROJECT_ROOT/log/rebot_eval_M_pseudo_mini${SUFFIX}" \
    --video_save \
    --save_gt
