#!/usr/bin/env bash
# Train ReBotNet S variant on gelsight_pseudo_mini transferred data in residual mode.
# Usage: bash train_refine_scripts/train_rebot_pseudo_mini_residual/train_rebot_S.sh <gpu_id> [matcher] [masked]
...
GPU=${1:?Usage: bash train_rebot_S.sh <gpu_id> [matcher] [masked]}
MATCHER="${2:-loftr}"
USE_MASK="${3:-}"
MODEL_SIZE=rebot_S
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/train.py" \
    ...
    --save_dir      "$PROJECT_ROOT/log/rebot_checkpoints_S_pseudo_mini${NAME_SUFFIX}" \
