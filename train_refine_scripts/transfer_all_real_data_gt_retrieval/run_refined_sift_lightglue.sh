#!/usr/bin/env bash
# Same as run_sift_lightglue.sh (sift_lightglue, modality=curvature, scale=8),
...
#
# Usage: bash run_refined_sift_lightglue.sh
...
# override e.g. `NUM_THREADS=7 bash run_refined_sift_lightglue.sh` when running 4-way in parallel on 28 cores.
...
OUT_BASE="$PROJECT_ROOT/log/transfer_pipeline_real_data_gt_retrieval_refined_sift_lightglue"
...
for session_dir in "$SESSIONS_BASE"/*/; do
    ...
    python "$PIPELINE_SCRIPT" \
        ...
        --transfer_matcher   sift_lightglue \
        --video_type         shadow \
        --photometric_refine \
