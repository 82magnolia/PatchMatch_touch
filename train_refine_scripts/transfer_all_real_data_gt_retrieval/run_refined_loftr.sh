#!/usr/bin/env bash
# Same as run.sh (loftr, modality=curvature, scale=8), but with
# --photometric_refine enabled (--photometric_refine_loss gradient).
#
...
#
# Usage: bash run_refined_loftr.sh
...
# override e.g. `NUM_THREADS=7 bash run_refined_loftr.sh` when running 4-way in parallel on 28 cores.
...
OUT_BASE="$PROJECT_ROOT/log/transfer_pipeline_real_data_gt_retrieval_refined_loftr"
