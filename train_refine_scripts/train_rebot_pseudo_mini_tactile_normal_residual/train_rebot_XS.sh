#!/usr/bin/env bash
# Train ReBotNet XS variant on gelsight_pseudo_mini tactile_normal-domain
...
# Usage: bash train_refine_scripts/train_rebot_pseudo_mini_tactile_normal_residual/train_rebot_XS.sh <gpu_id> [matcher]
...
GPU=${1:?Usage: bash train_rebot_XS.sh <gpu_id> [matcher]}
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/train.py" \
    ...
    --save_dir      "$PROJECT_ROOT/log/rebot_checkpoints_XS_pseudo_mini_tactile_normal_residual${SUFFIX}" \
