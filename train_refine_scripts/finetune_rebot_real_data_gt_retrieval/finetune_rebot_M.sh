#!/usr/bin/env bash
# Fine-tune a pseudo_mini-pretrained ReBotNet M variant on real-data
# (transferred -> query) pairs from the GT-retrieval pipeline.
#
# Loads the synthetic-pretrained checkpoint produced by
# train_refine_scripts/train_rebot_pseudo_mini/train_rebot_M.sh and adapts it to the
...
#
# Usage: bash train_refine_scripts/finetune_rebot_real_data_gt_retrieval/finetune_rebot_M.sh <gpu_id> [matcher] [refined] [finetune_mode]
...
GPU=${1:?Usage: bash finetune_rebot_M.sh <gpu_id> [matcher] [refined] [finetune_mode]}
MATCHER="${2:-loftr}"
REFINED="${3:-}"
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/finetune.py" \
    --pretrained    "$PROJECT_ROOT/log/rebot_checkpoints_M_pseudo_mini${CKPT_SUFFIX}" \
    --transfer_dir  "$PROJECT_ROOT/log/transfer_pipeline_real_data_gt_retrieval${TRANSFER_SUFFIX}" \
    --save_dir      "$PROJECT_ROOT/log/rebot_finetune_M_real_data_gt_retrieval${TRANSFER_SUFFIX}_${FINETUNE_MODE}" \
    ...
    --wandb_run_name "finetune_M_real${TRANSFER_SUFFIX}_${FINETUNE_MODE}_bs8_lr5e-5"
