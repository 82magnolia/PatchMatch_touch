#!/usr/bin/env bash
# Pretrain a CONDITIONED ReBotNet XS (residual) on gelsight_pseudo_mini in sim.
...
#
# Usage: bash train_refine_scripts/train_rebot_pseudo_mini_residual_cond/train_rebot_XS.sh <gpu_id> [matcher] [cond_mode] [data_mode]
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/train.py" \
    ...
    --save_dir      "$PROJECT_ROOT/log/rebot_checkpoints_XS_pseudo_mini${CKPT_NAME}_cond-${COND_MODE}${DATA_SUFFIX}" \
    ...
    --wandb_run_name "${MODEL_SIZE}_pseudo_mini${CKPT_NAME}_cond-${COND_MODE}${DATA_SUFFIX}_bs8_lr2e-4"
