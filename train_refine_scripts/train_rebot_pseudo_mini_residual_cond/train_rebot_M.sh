...
#
# Usage: bash train_refine_scripts/train_rebot_pseudo_mini_residual_cond/train_rebot_M.sh <gpu_id> [matcher] [cond_mode]
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/train.py" \
    ...
    --save_dir      "$PROJECT_ROOT/log/rebot_checkpoints_M_pseudo_mini${CKPT_NAME}_cond-${COND_MODE}" \
