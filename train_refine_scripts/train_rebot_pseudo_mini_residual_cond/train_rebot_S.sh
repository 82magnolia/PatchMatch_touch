#!/usr/bin/env bash
# Pretrain a CONDITIONED ReBotNet S (residual) on gelsight_pseudo_mini in sim.
...
#
# Usage: bash train_refine_scripts/train_rebot_pseudo_mini_residual_cond/train_rebot_S.sh <gpu_id> [matcher] [cond_mode] [data_mode]
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/train.py" \
    ...
    --save_dir      "$PROJECT_ROOT/log/rebot_checkpoints_S_pseudo_mini${CKPT_NAME}_cond-${COND_MODE}${DATA_SUFFIX}" \
