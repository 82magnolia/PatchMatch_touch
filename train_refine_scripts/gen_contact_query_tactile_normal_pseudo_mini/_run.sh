#!/usr/bin/env bash
# Worker: generates query tactile_normal videos (240×320, gelsight_pseudo_mini
# calibration) for 1/6 of all objects. Renders only the tactile_normal modality
# (no sim/shadow) -- see gen_contact_query_multi_pseudo_mini for the full sim+shadow variant.
...
OUT_BASE="$PROJECT_ROOT/Taxim/results/gen_contact_full_query_tactile_normal_pseudo_mini"
...
for obj_dir in "$OBJ_DIR"/*/; do
    ...
    echo "[${idx}] ($done_count/$total) [worker $WORKER_ID | GPU $GPU_ID] Generating query tactile_normal video → $save_dir"
    python "$GEN_SCRIPT" \
        --obj_path "$obj_path" \
