...
# Usage: bash train_refine_scripts/train_rebot_pseudo_mini/train_rebot_L.sh <gpu_id> [matcher] [masked]
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/train.py" \
    ...
    --save_dir      "$PROJECT_ROOT/log/rebot_checkpoints_L_pseudo_mini${SUFFIX}" \
    ...
    --wandb_run_name "${MODEL_SIZE}_pseudo_mini${SUFFIX}_bs4_lr2e-4"
