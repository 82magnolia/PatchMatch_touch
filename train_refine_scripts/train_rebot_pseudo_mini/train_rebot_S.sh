#!/usr/bin/env bash
# Train ReBotNet S variant on gelsight_pseudo_mini transferred data.
# Usage: bash train_refine_scripts/train_rebot_pseudo_mini/train_rebot_S.sh <gpu_id> [matcher] [masked]
...
CUDA_VISIBLE_DEVICES=$GPU python "$PROJECT_ROOT/rebot_net/train.py" \
    ...
    --save_dir      "$PROJECT_ROOT/log/rebot_checkpoints_S_pseudo_mini${SUFFIX}" \
