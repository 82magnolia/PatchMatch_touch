* General rules for writing  
  ...
  * Add references to tables and figures when necessary  
* Title  
...
* Abstract  
  * We introduce tactile analogies, an example-based synthesis paradigm for tactile videos  
  * Complete the abstract based on the introduction  
* Introduction  
  * Humans possess the remarkable ability to imagine what it would feel like to touch an object at a particular location  
    * This ability is crucial for grasp planning or obstacle avoidance, and holistically perceive the fine-grained surface details of the surrounding scene without manually touching every possible location  
    * Here, the notion of touch is not merely a snapshot in time, but rather a mental simulation of how the non-rigid skin would deform under contact  
  * Why is this possible?  
    * Tactile information is highly repetitive and predictable  
  * In this paper, we aim to teach machines the same capacity  
    * We introduce the task of finding tactile analogies  
    * Given a set of reference touches captured on an object during contact, we aim to predict the tactile signals at new, untouched locations  
    * Here, we represent touches as tactile normal videos which encode the gel normal deformations of visuo-tactile sensors over time  
      * Tactile normal videos could easily be obtained in practice as the mapping from RGB measurements to the normals is straightforward due to the design of modern visuo-tactile sensors  
      * Tactile sensor manufacturers such as Gelsight provide RGB-to-gel normal conversion pipelines as an API  
    * To predict gel normals, we render normal maps at both the reference and query tactile sensor poses in multiple scales, and compare these normals to find a mapping that can transfer reference tactile videos to query locations  
    * This approach is akin to the *analogy-based image synthesis* proposed in the seminal *Image Analogies* paper, where given a set of source images undergoing an unknown transformation (e.g., in style or texture), the goal is to synthesize for a target image how its transformed version would look like.  
      * Our paper’s task could be seen as an extension to this task, where the source images are the normal maps at each tactile sensor location, and the target “images” are the high-dimensional tactile videos that we aim to generate   
      * Rephrase the sentences on this bullet point since it is copied from the related works section  
  * Numerous challenges  
    * Complicated gel deformation physics that occur during contact: modeling this without knowledge of physical properties of the gel is challenging  
    * Large search space: need to search for which reference touch is the most relevant, and how the selected reference touch should map to the query sensor pose  
    * Lack of large-scale data for training  
  * Our solution  
    * To bridge between reference and untouched query locations, we use normal renderings at multiple scale  
    * Our solution is based on a coarse-to-fine estimation scheme  
      * Handle the large search space problem   
    * For coarse alignment, we use pre-trained local feature matchers on the normal map renderings, which despite being out-of-distribution of the multi-view RGB imagery the matchers are trained on, show highly accurate match success rates  
      * The local feature matches are used to find an affine map that aligns the two normal maps (reference, query), which is further used to warp the tactile videos to produce an initial transfer  
    * For refinement, we purpose training a refinement network that takes initially warped tactile normal videos and is trained to regress the ground-truth query tactile video  
      * The network is responsible for learning the subtle gel deformation dynamics and correct the mis-alignments from the initial transfer  
  * Experiment summary  
    * Our pipeline outperforms all the tested baselines  
    * Our pipeline exhibits a small runtime  
  * Applications in 3D reconstruction and visuo-tactile sensor simulation  
    * Could be used for fine-grained surface understanding and help embodied agents pre-trained on visuo-tactile sensor inputs  
  * Figures  
    * A one-column figure that will be the teaser, similar to the teaser image from the Image Analogies paper  
      * Link: [https://mrl.cs.nyu.edu/publications/image-analogies/analogies-72dpi.pdf](https://mrl.cs.nyu.edu/publications/image-analogies/analogies-72dpi.pdf)  
    * The caption will explain the setup of our method  
* Related Work  
  * Tactile Perception  
    ...
    * Touch localization  
      * As tactile sensors can provide fine-grained information about material / surface geometry, use the sensor measurements to localize the sensor with respect to an image or pre-built 3D map  
  ...
  * Predicting Sensory Signals from Examples  
    * Vision  
      * A large number of prior works exist for example-based image synthesis. Here, given a set of source images undergoing an unknown transformation (e.g., in style or texture), the goal is to synthesize for a target image how its transformed version would look like.   
        * Early approaches find this mapping using classical image matching  
          * Image analogies  
          * PatchMatch: a randomized correspondence algorithm for structural image editing  
        * Modern approaches use diffusion models or estimate dense correspondences between the source images using learned descriptors and matchers  
          * High-Resolution Image Synthesis with Latent Diffusion Models  
          * LightGlue: Local Feature Matching at Light Speed  
          * ASIC: Aligning Sparse in-the-wild Image Collections  
      * Our paper’s task could be seen as an extension to this task, where the source images are the normal maps at each tactile sensor location, and the target “images” are the high-dimensional tactile videos that we aim to generate   
    * Audio and Touch  
      * Inspired from the large body of works in example-based synthesis for images, there have been recent attempts to synthesize other sensory signals given reference samples.  
        * For audio, recent works try to generate audio spectrograms by jointly using video frames and reference audio data  
          * Conditional generation of audio from video via foley analogies  
          * FoleyCrafter: Bring Silent Videos to Life with Lifelike and Synchronized Sounds  
          * Audio Texture Manipulation by Exemplar-Based Analogy  
        * For touch, recent works mainly try to generate a single tactile image during contact from reference touch data  
          * Tactile DreamFusion: Exploiting Tactile Sensing for 3D Generation  
            * Use image quilting to tile a single tactile sensor measurement to other regions within an object  
          * Tactile-Augmented Radiance Fields (TaRF)  
            * Train a diffusion model on reference touch data that is conditioned on the nearby image captures to predict tactile images at new locations  
          * ObjectFolder: A Dataset of Objects with Implicit Visual, Auditory, and Tactile Representations, ObjectFolder 2.0: A Multisensory Object Dataset for Sim2Real Transfer  
            * Train an implicit neural representation that receives pixel coordinates \+ pressing depth as inputs and outputs the tactile image at that particular location. Trained on a per-object basis.  
...
* Method  
  * Overview  
    * As shown in Figure X, our method takes as input i) a set of reference tactile normal videos along with normal maps rendered at the same tactile sensor position, and ii) query tactile sensor poses to predict tactile normal videos  
      * We render normal maps at query sensor poses to use as context for tactile video prediction  
      * In practice, we can obtain view-space normals by i) obtaining depth maps from commodity sensors such as stereo cameras, ii) taking the gradients of the depth map to get a normal map in the camera space, and iii) projecting the normal maps to the tactile sensor space assuming the relative poses between the tactile sensor and camera could be known using forward kinematics (if the tactile sensor and camera is mounted on a robot) or fiducial markers (attach markers such as ARuCO at the back of the tactile sensor)  
    * Our method then takes a coarse-to-fine approach for transferring tactile videos, where we  
      * Retrieve the most promising reference touch location using DINOv3 features  
      * Coarsely align the normal maps to get an affine map that initially transfers the reference tactile to the query frame  
      * Refine the initial transfer results with a neural network  
  * Retrieval  
    * We extract DINOv3 features using the normal maps at higher scale than the sensor size  
    * Then, we obtain the top-1 retrieval by comparing DINOv3 features at each reference touch locations against that of the query location  
  * Coarse Alignment  
    * Using the top-1 retrieval result, we apply local feature matching to find a set of local feature correspondences between the normal maps of the top-1 reference and query  
    * Than using the 2D pixel correspondences, we estimate an affine map  
    * The affine map is used to warp the tactile video to obtain an initial transfer  
  * Learning-based Refinement  
    * Taking the initially transferred videos as input, our final step refines this initial transfer with a neural network  
    * As we aim to perform fast tactile video prediction suitable for AR/VR and robotics tasks, we balance refinement accuracy with efficiency  
    * Neural network architecture  
      * Our refinement network is inspired from RebotNet, which is a light-weight neural network for video enhancement  
      * However, RebotNet was originally developed to enhance blurs and compression artifacts from video  
      * We adapt RebotNet in the following ways  
        * Normal map concatenation  
          * Condition the network so that it is aware of the rough surface geometry of the query location  
        * Temporal conditioning with FiLM  
          * Normal maps are static: they are generated once at the query sensor pose  
          * Inform the network about which timestamp it is refining  
    * Training loss: Charbonnier loss  
  * Instructions  
    * Our method uses the following, so refer to the code described below for writing the method section  
    ...
    * Add equations after looking through the code above  
    * Avoid being too descriptive about the details  
  * Figures  
    * A one-column figure that shows how our method works, step-by-step with three subfigures  
      * One subfigure for retrieval  
        * Draw a database of reference touches \+ normal maps  
        * Show DINOv3 feature extractions being compared against query, and finally 1 getting selected  
      * One subfigure for coarse alignment  
      * One subfigure for neural network-based refinement  
        * Show the rough architecture of the refinement network  
        * No need to draw every detail  
        * Draw normal map concatenation and temporal conditioning with FiLM  
* Experiments  
  * Experimental Setup  
